# sr-barbara-module-split.md

**Path:** docs/changedocs/sr-barbara-module-split.md
**Syntax:** markdown
**Generated:** 2026-05-03 16:07:45

```markdown
# Sr. Barbara's Class — Changeset: game.js Module Split
**Date:** 2026-04-30
**Affects:** `static/js/` (six new files), `templates/index.html` (script tags),
`static/js/game.js` (deleted after split is verified)
**Reason:** Split monolithic `game.js` into focused modules. Each module
does one thing. Config is isolated so it can be replaced by the build
pipeline later.
...
| File | Responsibility |
|------|---------------|
| `config.js` | All constants and lookup tables |
| `layout.js` | Canvas geometry calculations |
| `renderer.js` | SVG drawing functions |
| `ui.js` | DOM manipulation, legend, feedback, popup |
| `data.js` | Flask API fetch |
| `main.js` | State, event wiring, app initialization |

State variables (`currentSentence`, `drawnRoles`, etc.) are declared as
globals in `main.js`. All modules reference them directly — no module
system, no imports. This matches the existing vanilla JS approach and
keeps the Slingcode build simple.

Load order matters: config → layout → renderer → ui → data → main.

---

## File 1: `static/js/config.js` (new file)
...
## File 2: `static/js/layout.js` (new file)
...
## File 3: `static/js/renderer.js` (new file)
...
## File 4: `static/js/ui.js` (new file)
...
## File 5: `static/js/data.js` (new file)
...
## File 6: `static/js/main.js` (new file)
...
## File 7: `templates/index.html` (modified)

Replace the single script tag at the bottom of `<body>`:
...
1. Create the six new files in `static/js/`.
2. Update `templates/index.html` script tags.
3. Hard refresh the browser and test — do not delete `game.js` until verified.
4. Once verified, delete `game.js`.

### Commit message

```
refactor: split game.js into config, layout, renderer, ui, data, main
```
...
All state variables (`currentSentence`, `drawnRoles`, etc.) and DOM
references are declared inside the `DOMContentLoaded` callback in
`main.js`. Functions in other modules reference these as closed-over
variables — they work because the other modules' functions are called
from within `main.js`'s closure scope, where those variables are
visible.

This is the same pattern as the original monolith. No module system,
no imports — consistent with the vanilla JS approach and compatible
with the Slingcode build.
